...
const Cart = () => {
  ...
  return (
    <Container className="py-4 mt-5">
      <h1 className={`${theme ? "text-light" : "text-light-primary"} my-5 text-center`}>{isEmpty ? "Keranjang Belanja Kosong" : "Keranjang Transaksi"}</h1>
            ...
            {items.map((item, index) => {
              return (
                <tr key={index}>
                  ...
                  <td>$ {item.price}</td>
                  <td>Kuantiti ({item.quantity})</td>
                  <td>
                    <Button onClick={() => updateItemQuantity(item.id, item.quantity - 1)} className="ms-2">
                      -
                    </Button>
                    <Button onClick={() => updateItemQuantity(item.id, item.quantity + 1)} className="ms-2">
                      +
                    </Button>
                    <Button variant="danger" onClick={() => removeItem(item.id)} className="ms-2">
                      Hapus
                    </Button>
                  </td>
                </tr>
